...
public class QualificationViewModel {
  ...
  public QualificationViewModel(ServiceNuits service) {
    ...
    descriptionSelection.bind(
        Bindings.createStringBinding(
            () -> {
              Sequence sequence = sequenceSelectionnee.get();
              ...
              return String.format(
                  "Séquence %s - %.1f kHz",
                  sequence.getHorodatage().format(HEURE), sequence.getFrequenceDominanteKHz());
            },
    ...
    verdictGlobalLibelle.bind(
        Bindings.createStringBinding(
            () -> {
              if (verdictSaisi.isEmpty().get()) return "Verdict global : (à saisir)";
              return "Verdict global : " + verdictSaisi.get();
            },
  ...
  public void ecouterCommand() {
    ...
    Sequence sequence = sequenceSelectionnee.get();
    if (sequence != null) sequence.setStatut("Écoutée");
  }
